...
Document version 0.9, Released 2023-04-23  
...
mylaptop.backup/                        archive dir
    ...
    ├── archive.ini.salt                archive encryption salt
...
New or changed metadata is saved initially with a '.tmp' extension for the object in
question, and then "zippered" up the hierarchy to _archive.ini.tmp_ with updated hashes for
each changed file.  The '.tmp' files are then renamed without the extension to replace the existing
metadata.  For a Wyng `send` operation, the following metadata files must be created or updated:
_manifest.z, info, volinfo, archive.ini_.
...
The KDF used is `scrypt(passphrase, salt, n=2^19, r=8, p=1)`.

Key rotation / PFS is _not_ implemented and both keys are static.  However, the cipher's 192-bit
nonces are used to their full extent with each nonce being a concatenation of 32-bit Unix current
time, 80 random bits, and 80-bit counter.  The counter for each key is mirrored between the
_archive.ini_ and the local cache of the _archive.ini.salt_, with the highest value of the two
used at runtime.
...
|'hashtype = '     | str     | Hash type,          Ex. _blake2b_
|'vgname = '       | str     | Local volume group, Ex. _vg1_ or blank
|'poolname = '     | str     | Local storage pool name or path, Ex. _pool1_ or _/mnt/backups_
|'data_cipher = '  | str     | Cipher for data,    Ex. _chacha20_
|'dataci_count = ' | int     | Counter for data cipher
|'mci_count = '    | int     | Counter for metadata cipher
|                  | _blank_ |
|'[volumes]'       |         | ini section header: volumes list
|vid[0] ' = '      | 256bit_b64_str | Vol id & volinfo file hash
|vid[N] ' = '      | 256bit_b64_str | Vol id & volinfo file hash [...]
|                  | _blank_ |
...
Holds two key salts with counters, one set for data (slot=0) and one set for metadata (slot=1). The offset for
a key slot is calculated as: `(keybits*2//8 + max_ct_bits//8) * slot`.


| Bytes  | Type    | Desc |
|:-------|:--------|:-----|
| 10     | int     | 80-bit counter
| 32     | binary  | 256-bit key salt
...
|session[0] ' = ' | 256bit_b64_str | Session name & info file hash
|session[N] ' = ' | 256bit_b64_str | Session name & info file hash [...]
...
Each data chunk file is named for the in-volume address it represents.  It contains compressed and
encrypted series of data blocks from a source volume, which is validated by the
corresponding hash in _manifest.z_.

The size of each chunk before compression and encryption is uniform for each archive, with the
choices at archive creation being _64KiB / 2 * (2^N)_, where _N_ is an integer from 1 to 6.
