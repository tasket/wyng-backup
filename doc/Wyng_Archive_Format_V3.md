...
Document version 0.9.2, Released 2023-06-18  
...
_archive.ini.salt_, with the highest value of the two used at runtime.  Specifics of each
...
Holds four key salt + counter pairs, with the first two sets for data (slot 0) and metadata (slot 1). Slots 2 and 3 provide salts for derived subkeys for 'Wyng_Nonces' and 'Wyng-Manifest-Hash' contexts, respectively.  The offset for a key slot is calculated as: `(16 + 64) * slot`.

The key derivation function is `scrypt(passphrase, salt, n=2**19, r=8, p=1, maxmem=640*1024*1024, keysize)`.  The subkey derivation function is `HKDF(key, size, salt, 'SHA512', slot-2, context)`.
